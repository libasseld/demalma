<div>
        ...
        <div class="grid grid-cols-3 gap-3">
            ...
            <div class="panel-white rounded-md mt-4 shadow-xl border-gray-100 border-2 p-4 ">
                ...
                @if ($current_agent_demande && $current_agent_demande->acceptee == 0)
                    <div class="h-full flex flex-col justify-center">
                        ...
                        <div class="flex justify-around mt-2">
                            <button class="btn btn-outline-success " wire:click="show_modal_acceptation(1)"> Accepter ? </button>
                            <button class="btn btn-outline-danger " wire:click="show_modal_acceptation(-1)"> Ne pas accepter ? </button>
                        </div>
                    </div>
                @else
                <table>
                    <tr>
                        ...
                        <td class="p-2">{{!isset($demande->agent_traitement->user) ? 'Aucun' : $demande->agent_traitement->user->name }}</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td class="p-2">{{!isset($demande->agent_depot->user) ? 'Aucun' : $demande->agent_depot->user->name }}</td>
                        <td>
                    ...
                    <tr>
                        ...
                        <td class="p-2">{{!isset($demande->agent_livraison->user) ? 'Aucun' : $demande->agent_livraison->user->name }}</td>
                        <td>
                ...
                @if(Auth::user()->role->code == 'superviseur')
                    <button type="button" wire:click="$set('show_modal_agents', true)" class="btn btn-warning btn-sm mt-2 float-right">Modifier</button>
                @endif
                @endif
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-[999] hidden overflow-y-auto bg-[black]/60 !block" :class="open &amp;&amp; '!block'">
        <div class="flex min-h-screen items-start justify-center px-4">
            <div x-show="open" x-transition="" x-transition.duration.300="" class="panel my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0">
                <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                    ...
                    <button type="button" class="text-white-dark hover:text-dark" wire:click="$set('show_modal_agents', false)">
                ...
                <div class="p-5">
                    <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                        <div class="mt-4" wire:ignore>
                            ...
                            <select id="agent_traitement_id" wire:model="agent_traitement_id" class="form-select text-white-dark select-2" required="">
                        ...
                        <div class="mt-4" wire:ignore>
                            ...
                            <select id="agent_depot_id" wire:model="agent_depot_id" class="form-select text-white-dark select-2" required="">
                                <option value="">Sélectionner un agent</option>
                        ...
                        <div class="mt-4" wire:ignore>
                            ...
                            <select id="agent_livraison_id" wire:model="agent_livraison_id" class="form-select text-white-dark select-2" required="">
                                <option value="">Sélectionner un agent</option>
                    ...
                    <div class="mt-8 flex items-center justify-end">
                        <button type="button" class="btn btn-outline-danger" wire:click="$set('show_modal_agents', false)">Annuler</button>
                        <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4" wire:click="save_agents()">Enregistrer</button>
                    </div>
                </div>
            </div>
        </div>
...
<div class="fixed inset-0 z-[999]  overflow-y-auto bg-[black]/60 !block" :class="open &amp;&amp; '!block'">
    <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
        <div class="panel animate__animated animate__zoomInUp my-8 w-full max-w-lg overflow-hidden rounded-lg border-0 p-0">
            <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                <h5 class="text-lg font-bold">Acceptation de la demande</h5>
                <button type="button" class="text-white-dark hover:text-dark" wire:click="$set('modal_acceptation', false)">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="h-6 w-6">
                        <line x1="18" y1="6" x2="6" y2="18"></line>
                        <line x1="6" y1="6" x2="18" y2="18"></line>
                    </svg>
            ...
            <div class="p-5">
                <div class="text-base font-medium text-[#1f2937] dark:text-white-dark/70">
                    <p>
                       Vous êtes sur le point <strong>{{$value_accept == 1 ? " d'accepter ": "de rejeter" }}</strong> cette demande.
                    </p>
                    <p>
                ...
                <div class="mt-8 flex items-center justify-end">
                    <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4" wire:click="$set('modal_acceptation', false)" >Ferner</button>
                    @if ($value_accept == 1)
                        <button type="button" class="btn btn-success ltr:ml-4 rtl:mr-4" wire:click="save_acceptation">Accepter </button>
                    @else
                        <button type="button" class="btn btn-danger ltr:ml-4 rtl:mr-4" wire:click="save_acceptation">Ne pas accepter</button>
                    @endif
                </div>
            </div>
        </div>
